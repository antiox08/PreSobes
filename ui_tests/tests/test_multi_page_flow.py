...
def test_contact_sales_form_is_filled(page):
    with allure.step(f"Open GitHub homepage"):
        home = HomePage(page)
        home.open()

    with allure.step(f"Open solutions page"):
        home.go_to_solutions()

    with allure.step(f"Click cicd link"):
        solutions_menu = SolutionsMenu(page)
        solutions_menu.select_cicd()

    with allure.step(f"Open contact sales page"):
        cicd = CiCdPage(page)
        cicd.click_contact_sales()

    with allure.step(f"Fill form contact sales page"):
